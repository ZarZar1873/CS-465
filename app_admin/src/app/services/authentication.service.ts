import { Inject, Injectable } from '@angular/core'; // manage access to the Storage provider
import { BROWSER_STORAGE } from '../storage'; // provides access to local storage provider
import { User } from '../models/user'; // provides means of representing user data
import { AuthResponse } from '../models/auth-response'; // provides representation for JWT
// needed as the serice will be modified to add login and register endpoints
...
export class AuthenticationService {
  ...
  constructor (
    // Inject storage provider so that its data contents are persistent across any modules or components
    @Inject(BROWSER_STORAGE) private storage: Storage,
    // Wire up TripDataService so that we can communicate with our API endpoints
